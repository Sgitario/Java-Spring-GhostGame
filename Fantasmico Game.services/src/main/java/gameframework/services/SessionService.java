...
public class SessionService implements ISessionService
{
    ...
    public String registerSession(String game, String lang, Integer level, Map<String, String> properties)
    {
        ...
        if (this.sessionRepository.storeSession(session)) {
            GameLogger.debug(String.format("Session was properly stored: %s.", session.toString()));
        } else {
            // Session not stored
            GameLogger.warn(String.format("Session was not stored properly: %s.", session.toString()));
    ...
    {
        sessionRepository.deleteSession(session.getGameName(), session.getToken());

        sessionRepository.storeSession(session);

        GameLogger.debug(String.format("Session was properly updated: %s.", session.toString()));
    }
