...
/**
 * The optimal ghost strategy works by following the next strategy: If there are several winning moves, the computer
 * will choose one randomly. If there are no winning moves, the computer will choose one among the longest word randomly
...
public class OptimalGhostStrategy implements IGhostStrategy
{
     ...
     */
    public String addLetter(final List<String> choices, final String currentString)
    {
        // Retrieve all possible letters.
        Map<String, List<String>> letters = this.retrieveLetterChoices(choices, currentString);

        // Look for the letters that satisfy the conditions for winning the game
        List<String> lettersMoves = this.satisfyOptimalSolution(letters, currentString);
     ...
     * @return
     */
    private Map<String, List<String>> retrieveLetterChoices(List<String> choices, String currentString)
    {
        Map<String, List<String>> letters = new HashMap<String, List<String>>();

        for (String choice : choices) {
            if (isChoice(choice, currentString)) {
                String letter = choice.substring(currentString.length(), currentString.length() + 1);
        ...
        GameLogger.debug(String.format("Letters %s for current string %s.", letters, currentString));
     ...
     * @return
     */
    private List<String> satisfyOptimalSolution(Map<String, List<String>> letters, String currentString)
    {
        ...
        for (Entry<String, List<String>> node : letters.entrySet()) {
            ...
            // If all future choices have pair length and there is no words, it is a optimal solution.
            if (allChoicesOddLength) {
                optimalLetters.add(node.getKey());
            }
        }

        GameLogger.debug(String.format("Winning moves %s for current string %s.", optimalLetters, currentString));
     ...
     * @return
     */
    private List<String> retrieveLongestChoices(final Map<String, List<String>> letters, final String currentString)
    {
        ...
        for (Entry<String, List<String>> node : letters.entrySet()) {
            ...
            for (String choice : node.getValue()) {
                // If the choice has the same length that the longest one, then add it. If not, reset.
        ...
        GameLogger.debug(String.format("Longest moves %s for current string %s.", longestLetters, currentString));

        return longestLetters;
    }

    /**
     * The list should have filtered the choices except when the minimal length it has not achieved yes. Therefore, it
